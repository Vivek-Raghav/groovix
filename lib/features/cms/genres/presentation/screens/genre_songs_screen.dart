...
class _GenreSongsScreenState extends State<GenreSongsScreen> {
  ...
  void initState() {
    super.initState();
    getIt<CmsGenreBloc>().add(FetchGenreSongs(widget.genre.id));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<CmsGenreBloc, CmsGenreState>(
      listener: (context, state) {
        if (state is RemoveSongsFromGenreLoaded) {
          ...
          getIt<CmsGenreBloc>().add(FetchGenreSongs(widget.genre.id));
        } else if (state is RemoveSongsFromGenreError) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
      ...
      child: Scaffold(
        ...
        body: BlocBuilder<CmsGenreBloc, CmsGenreState>(
          builder: (context, state) {
            if (state is GetGenreSongsLoading) {
              return const Center(
                child:
            ...
            if (state is GetGenreSongsError) {
              return Center(
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        context
                            .read<CmsGenreBloc>()
                            .add(FetchGenreSongs(widget.genre.id));
                      },
            ...
            if (state is GetGenreSongsLoaded) {
              ...
              return Stack(
                children: [
                  ...
                  if (state is RemoveSongsFromGenreLoading)
                    Container(
  ...
  void _showDeleteConfirmation() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to remove ${_selectedSongIds.length} song${_selectedSongIds.length > 1 ? 's' : ''} from "${widget.genre.name}"?',
        ),
        actions: [
          TextButton(
  ...
  void _removeSongs() {
    ...
    getIt<CmsGenreBloc>().add(
      RemoveSongsFromGenre(widget.genre.id, request),
    );
  }
}
