...
class SongRepositoryImpl extends SongRepository {
  ...
  @override
  EitherDynamic<List<SongModel>> getRecentSongs() async {
    try {
      final data = await songRemoteDataSource.getRecentSongs();
      if (data.isNotEmpty) {
        return Right(data);
      } else {
        return Left(
  ...
  @override
  EitherDynamic<List<SongModel>> getLikedSongs() async {
    try {
      final data = await songRemoteDataSource.getLikedSongs();
      if (data.isNotEmpty) {
        return Right(data);
      } else {
        return Left(
